 ...
 * 1- The idea here is that when we see an open bracket, we push it to stack.
 * 2- If we see a closed bracket and it is not matching with the top of stack or stack is empty then the string is not valid.
 ...
 *  Space Complexity is O(n) as we push all opening brackets onto the stack and in the worst case, we will end up pushing
 ...
 *  if the string has only one closed bracket or starting with closed bracket "}"
 *
 **/
